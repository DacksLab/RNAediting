...
summary.edit.table = read_csv('../DATA/pipeline_raw.csv')
...
# Number of codons (in the plastid genome)
num.codons.sp = NULL
# Number of edits
number.edition.sp = NULL
for (i in Short.names){
  t1=sum(subset(table,Organism==i)$amino.acid.length)
  t2=nrow(subset(summary.edit.table,Organism == i))
  num.codons.sp = c(num.codons.sp, t1)
  number.edition.sp = c(number.edition.sp, t2)
}
number.edition = sum(number.edition.sp)
num.codons = sum(num.codons.sp)
# Averaged position bias
position.rate = c(nrow(subset(summary.edit.table, codon.position == 1))/nrow(summary.edit.table),
                  nrow(subset(summary.edit.table, codon.position == 2))/nrow(summary.edit.table),
                  nrow(subset(summary.edit.table, codon.position == 3))/nrow(summary.edit.table))
...
for (num in 1:100){
  ...
  for (i in 1:number.edition){
    ...
    }else if (y > position.rate[1]  & y <= (position.rate[1]+position.rate[2])){
...
p1 = ggplot(plot.1, aes(x = number.per.codon, y = value, fill = type)) + 
...
p2 = ggplot(plot.2, aes(x = number.per.codon, y = value, fill = type)) + 
  geom_bar(stat='identity',position=position_dodge(width=.9), width = .8) + theme_gray() +
...
plot_save = plot_grid(p1,p2,nrow=1,labels='AUTO')
ggsave(filename = "../RESULTS/9_position.bias.pdf", plot_save, width=14, height=6)
##########
